...
#define SAMPLING_RADIUS 0.3
#define NUM_SAMPLING_DIRECTIONS 16
#define SAMPLING_STEP 0.004
#define NUM_SAMPLING_STEPS 16
...
void main()
{
	float start_Z = texture(depthMap, vec2(1.0 - uvCoords.x, uvCoords.y)).r; //fragment shader runs for each pixel 
	vec3 start_pos = vec3(vec2(1.0 - uvCoords.x, uvCoords.y), start_Z);
	...
	vec3 viewNorm = normalize(texture(gNormal, vec2(1.0 - uvCoords.x, uvCoords.y)).rgb * 2 - 1);
	float total = 0.0;
	float sample_direction_increment = 2 * M_PI / NUM_SAMPLING_DIRECTIONS; //split sampling sphere into multiple direction angles
